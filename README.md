...
- Ordering lives in `lib/challenge/ordering.ts`: exclude the 20 most recently shown distinct eligible words (or all but one for a small pool), then choose a least-seen word with random ties. Due mistakes take priority within that spaced pool.
- Missed/revealed words become due after 5–20 additional question presentations; the recent-word exclusion still applies, so review can be later. Retired questions count as presentations; reloads of a pending question do not. Existing progress and review thresholds remain compatible.
