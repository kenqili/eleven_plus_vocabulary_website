...
<p>Five correct answers master a word. Needs practice means an unmastered word with a mistake or revealed answer.</p>
${words.length ? words.map((word) => `<article><h2>${escape(word.word)}</h2><p class="progress">${DIFFICULTY_LEVELS[word.difficulty]} · ${LEARNING_STATUS[word.status]} · ${word.correct}/${MASTERY_TARGET} correct · ${word.mistakes} mistakes · ${word.reveals} reveals</p><p>${escape(word.definition)}</p>${word.example ? `<p><em>${escape(word.example)}</em></p>` : ""}<p><strong>Synonyms:</strong> ${escape(word.syn || "—")}<br><strong>Antonyms:</strong> ${escape(word.ant || "—")}</p><div class="notes">Notes:</div></article>`).join("") : "<p>No words match these filters.</p>"}
<footer><p>Difficulty levels 1–5 combine word length and general English frequency; they are estimates relative to this collection, not exam grades. Frequency and derived level data: <a href="https://github.com/rspeer/wordfreq">wordfreq 3.1.1 by Robyn Speer</a>, incorporating freely available SUBTLEX by Marc Brysbaert and colleagues. Derived data licensed <a href="https://creativecommons.org/licenses/by-sa/4.0/">CC BY-SA 4.0</a>.</p></footer>
</body></html>`;
